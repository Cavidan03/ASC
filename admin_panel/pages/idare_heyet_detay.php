...
if (isset($_POST['update'])) {
    ...
    $sql = $db->prepare('UPDATE `idare_heyet` SET `sekil` = ?, `ad` = ?, `hazirki_vezife` = ?, `e-mail` = ? WHERE  `idare_heyet`.`id` = ?');
    $params = [
        $picUrl,
        $_POST['ad'],
...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $loop = $db->prepare("SELECT * FROM `idare_heyet` WHERE id=:id");
    $loop->execute([':id' => $id]);
        ...
        <div class="container-fluid py-1 px-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-8 me-7">
                    <li class="breadcrumb-item text-sm"><a class="opacity-5 text-dark" href="idare_heyet.php">İdarə
                            Heyəti</a>
                    </li>
                    <li class="breadcrumb-item text-sm text-dark active" aria-current="page">İdarə Heyət Detayı</li>
                </ol>
                <h6 class="font-weight-bolder mb-0">İdarə Heyəti Detayı</h6>
            </nav>
